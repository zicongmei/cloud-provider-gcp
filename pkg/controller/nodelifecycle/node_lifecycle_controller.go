...
// GCEFilteringNodeInformer wraps a NodeInformer to filter out non-GCE nodes from the lister.
...
// GCEFilteringNodeLister wraps a NodeLister to filter out non-GCE nodes.
...
func (l *GCEFilteringNodeLister) List(selector labels.Selector) (ret []*v1.Node, err error) {
	...
	for _, n := range nodes {
		if strings.HasPrefix(n.Spec.ProviderID, providerPrefix) {
...
// NewCloudNodeLifecycleController returns a new cloud node lifecycle controller that filters for GCE nodes.
